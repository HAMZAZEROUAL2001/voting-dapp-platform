...
async function main() {
  ...
  console.log("=== Vérification Détaillée du Contrat de Vote ===");
  ...
  console.log(`L'administrateur est le compte de déploiement : ${admin === deployer.address}`);
...
Candidat ${i + 1}:
  ...
  console.log("\n=== Informations Supplémentaires ===");
  console.log(`Nombre de comptes disponibles : ${otherAccounts.length + 1}`);
  console.log("Adresses des comptes :");
  console.log(`  - Déployeur : ${deployer.address}`);
  otherAccounts.forEach((account, index) => {
    console.log(`  - Compte ${index + 2} : ${account.address}`);
  });
}
